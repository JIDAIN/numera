...
- HistoryCharts 尚未按 C project analyticsKey 分成正式趋势轨道。

因为当前还没有正式 C1～C4 用户入口，这项随首个 C 项目实现补齐。
...
C 当前尚无正式 project generator，因此普通 C“再来一组”暂时只能保留冻结题集语义。首个正式 C 项目接入时，必须把 project generator 接入 recreate 路径，保证“再来一组”生成新题。
